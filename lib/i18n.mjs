...
// Supported languages
const SUPPORTED_LOCALES = ['zh-CN', 'en', 'fr'];
...
export function getLanguage() {
  ...
  return normalizeLanguage(process.env.CRUCIX_LANG || process.env.LANGUAGE || process.env.LANG || DEFAULT_LOCALE);
}

/**
 * Normalize user-provided language codes to supported locale ids.
 * @param {string} lang
 * @returns {string}
 */
export function normalizeLanguage(lang) {
  const raw = String(lang || DEFAULT_LOCALE).trim().toLowerCase();
  if (raw === 'zh' || raw === 'zh-cn' || raw === 'zh_hans' || raw.startsWith('zh_cn')) return 'zh-CN';
...
export function getLocaleForLanguage(lang) {
  return loadLocale(normalizeLanguage(lang));
}
...
export function isSupported(lang) {
  return SUPPORTED_LOCALES.includes(normalizeLanguage(lang));
}
